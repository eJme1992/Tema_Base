...
class FSCF_Options {
	...
	static function register_options_page() {
		...
		add_plugins_page(
				__('FS Contact Form Options', 'si-contact-form'), // The text to the display in the browser when this menu item is active
...
//		echo "This is the basic settings section"; ?>
	...
	static function email_settings_callback() {
		...
		if ( $safe_mode_is_on ) {
			echo '<br /><span class="fscf_warning_text">' . __( 'Warning: Your web host has PHP safe_mode turned on.', 'si-contact-form' );
			echo '</span> ';
		...
		if ( phpversion() < 5 ) {
			echo '<br /><span class="fscf_warning_text">' . __( 'Warning: Your web host has not upgraded from PHP4 to PHP5.', 'si-contact-form' );
			echo '</span> ';
...
//		echo "This is the field settings section";
		...
		foreach ( self::$form_options['fields'] as $key => $field ) {
          ...
          if ( FSCF_EMAIL_FIELD == $field['standard'] && 'true' == self::$form_options['double_email'] && $field['default'] != '' ) {
			...
			<?php } ?>
			&nbsp;&nbsp;
			<input name="fscf_show_fields" type="button" id="button<?php echo $key; ?>" value="Show Details" class="button" onclick="toggleVisibilitybutton('<?php echo $key; ?>')" />
...
<br />
	...
	static function tooltip_settings_callback() {
		...
		<label for="<?php echo self::$form_option_name;?>[tooltip_captcha]"><?php _e('CAPTCHA Image', 'si-contact-form'); ?></label>
		...
		<label for="<?php echo self::$form_option_name;?>[tooltip_refresh]"><?php _e('Refresh Image', 'si-contact-form'); ?></label>
	...
	static function data_export_callback() {
  	...
  	static function captcha_settings_callback() {
		...
		<label for="<?php echo self::$form_option_name; ?>[captcha_small]"><?php _e( 'Enable smaller size CAPTCHA image.', 'si-contact-form' ); ?></label>
		...
		<label for="<?php echo self::$form_option_name; ?>[captcha_perm]"><?php _e( 'Hide CAPTCHA for', 'si-contact-form' ); ?>
